...
철학: "누가 이 코드를 쌌나(blame)"가 아니라 **"누가 이 코드를 가장 잘 해결할 수
...
def get_commit_history(repo_path: str, file_path: str) -> list[_Commit]:
    ...
    `git log --follow --numstat --format=%H|%an|%at` 한 번으로
    메타데이터와 변경량을 함께 파싱한다.
    """
    ...
    for line in out.splitlines():
        ...
        if "|" in line and "\t" not in line:
            # 헤더 라인: <hash>|<author>|<timestamp>
            parts = line.split("|")
            # author에 '|'가 들어갈 수 있으니 양끝(hash, ts)을 고정으로 본다.
            try:
                ts = int(parts[-1])
            except ValueError:
                cur = None
...
def _max_ccn_at(repo_path: str, commit_hash: str, path: str) -> int | None:
    ...
    try:
        with tempfile.NamedTemporaryFile(
            ...
            tmp.write(content)
            tmp_path = tmp.name
        info = lizard.analyze_file(tmp_path)
...
def compute_ownership(
    ...
    for commit in commits:  # 오래된 → 최신
        ...
        scores[commit.author] = scores.get(commit.author, 0.0) + commit_score
        counts[commit.author] = counts.get(commit.author, 0) + 1
        if commit.timestamp > last_ts.get(commit.author, 0):
            last_ts[commit.author] = commit.timestamp

    contributors = [
        ContributorScore(
            name=author,
            score=round(score, 4),
            commit_count=counts[author],
            last_commit_days=max(
                (now - datetime.fromtimestamp(last_ts[author], tz=timezone.utc)).days,
                0,
            ),
        )
