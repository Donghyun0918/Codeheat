...
├── codeheat/
│   ├── __init__.py
│   ├── models.py          # 완료: dataclass 데이터 모델
│   ├── static_scan.py     # 완료: 복잡도 + TODO 분석
│   ├── cli.py             # 완료: `codeheat scan <path>` 진입점
│   ├── ownership.py       # 미작성 (2단계)
...
### 2단계 — 오너십 분석 (`ownership.py`)
목표: 파일별로 "도메인 지식 점수"가 높은 기여자 1~2명 추출.
- `git log --follow --format='%H|%an|%at' -- <file>` 로 커밋 히스토리 수집
- 핵심 정의: 단순 최다 수정자가 아니라 **복잡도가 급증한 시점에 커밋한 사람**에 가중치
- 점수 = Σ(수정 횟수 × 최근성 가중치 × 변화량 가중치)
...
3. 다음 작업은 `ownership.py` 작성부터 (2단계)
